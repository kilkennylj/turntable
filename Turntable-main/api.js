...
exports.setApp = function (app, client)
{
	...
	async function getsuggestionsalbums({ albumIds}){
		const db = client.db("Turntable");
	...
	app.post('/api/getsuggestions', async(req, res, next) =>
	{
		...
		for (var i = 0; i < cleanSuggest.length; i++)
		{
			results[i] = await db.collection("Albums").findOne({ _id: new ObjectId(cleanSuggest[i])});
			results[i].rating = user.Ratings[i];
		}
		...
		res.status(200).json( { albums: results, error: error } );
	});
